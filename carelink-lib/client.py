...
class CareLinkClient(object):
    ...
    def _get_config(self, discovery_url, country):
        ...
        if config is None:
            raise Exception(
                "ERROR: failed to get config base urls for region %s" % region
            )
        ...
        sso_base_url = "https://%s:%d/%s" % (
            sso_config["server"]["hostname"],
    ...
    ###########################################################
    # Get data from carepartner endpoint (POST)
    ###########################################################
    def _get_data_carepartner(self, config, token_data, username, patientid, headers):
        log.info("_get_data_carepartner()")
        url = config["baseUrlCumulus"] + "/display/message"
        data = {"username": username, "role": "carepartner", "patientId": patientid}
        resp = requests.post(url=url, headers=headers, data=json.dumps(data))
        return resp

    ###########################################################
    # Get data from patient endpoint (GET)
    ###########################################################
    def _get_data_patient(self, config, token_data, headers):
        log.info("_get_data_patient()")
        # Extract base URL (e.g., https://clcloud.minimed.eu)
        base_url = config["baseUrlCumulus"].rsplit("/connect/carepartner", 1)[0]
    ...
    def _get_data(self, config, token_data, username, role, patientid):
        ...
        if role in ["CARE_PARTNER", "CARE_PARTNER_OUS"]:
            resp = self._get_data_carepartner(
                config, token_data, username, patientid, headers
            )
        else:
            resp = self._get_data_patient(config, token_data, headers)
    ...
    def _init(self):
        ...
        try:
            ...
            self.__username = self.__accessTokenPayload["token_details"][
                "preferred_username"
            ]
            ...
            if self.__last_api_status in AUTH_ERROR_CODES:
                try:
                    ...
                    self.__accessTokenPayload = self._get_access_token_payload(
                        self.__tokenData
                    )
    ...
    def printUserInfo(self):
        ...
        print(
            "   user:     %s (%s %s)"
            % (self.__username, self.__user["firstName"], self.__user["lastName"])
        )
        ...
        if self.__patient is not None:
            print(
                ...
                % (
                    ...
                    self.__patient["firstName"],
                    self.__patient["lastName"],
                )
            )
